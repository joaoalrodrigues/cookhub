...
export default function App() {
  const [activeTab, setActiveTab] = useState<'bakes' | 'recipes' | 'profile' | 'dashboard' | 'explore'>('dashboard');
    ...
    <div className="min-h-screen bg-[#FDFCF8] text-[#2C2115] font-serif selection:bg-[#E2D6C0]">
      ...
      <header className="border-b border-[#E2D6C0] bg-white/50 backdrop-blur-sm sticky top-0 z-50">
        <div className="max-w-4xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="hidden md:flex gap-1 text-sm font-sans uppercase tracking-[0.2em] font-bold items-center bg-[#FDFCF8] p-1 rounded-2xl border border-[#E2D6C0]">
            {[
              ...
              <button 
                ...
                className={`px-6 py-2 rounded-xl transition-all duration-300 text-[10px] ${
                  activeTab === item.id 
            ...
            <div className="w-px h-4 bg-[#E2D6C0] mx-2" />
            <button 
              onClick={() => setActiveTab('profile')}
              className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all duration-300 text-[10px] ${
                activeTab === 'profile' 
                  ? 'bg-[#5A5A40] text-white shadow-sm' 
                  : 'text-[#5A5A40]/40 hover:text-[#5A5A40] hover:bg-black/5'
              }`}
            >
              <div className="w-5 h-5 rounded-full overflow-hidden border border-current opacity-80">
      ...
      <main className="max-w-4xl mx-auto px-6 py-12">
        {activeTab === 'dashboard' ? (
          <Dashboard onCreateRecipe={() => setActiveTab('recipes')} />
        ) : activeTab === 'recipes' ? (
          <RecipeForm />
        ) : activeTab === 'explore' ? (
          <Explore />
      ...
      <footer className="mt-20 border-t border-[#E2D6C0] py-8 text-center text-xs opacity-40 uppercase tracking-widest font-bold font-sans">
        CookHub &copy; {new Date().getFullYear()} • De Padeiros para Padeiros
      </footer>
    </div>
  );
}
