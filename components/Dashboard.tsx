...
import TechniquesDrawer from "./TechniquesDrawer";
...
export default function Dashboard({ onCreateRecipe, onOpenRecipe, onEditRecipe, onOpenTimers }: DashboardProps) {
  const [isCostModalOpen, setIsCostModalOpen] = useState(false);
  const [isTechniquesDrawerOpen, setIsTechniquesDrawerOpen] = useState(false);
  ...
  return (
    <div className="space-y-12 pb-20">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {MOCK_DRAFTS.map((draft, idx) => (
            <motion.div 
              ...
              className="bg-white p-6 rounded-3xl border border-[#E2D6C0] flex items-center justify-between group hover:border-[#5A5A40] transition-colors cursor-pointer"
              onClick={() => onEditRecipe && onEditRecipe(draft.id)}
            >
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { icon: BookOpen, label: "Fichas Técnicas", color: "text-blue-500", onClick: () => setIsTechniquesDrawerOpen(true) },
          { icon: Clock, label: "Temporizadores", color: "text-orange-500", onClick: onOpenTimers },
          { icon: Trophy, label: "Desafios", color: "text-purple-500" },
          { icon: DollarSign, label: "Calculadora de Custos", color: "text-green-500", onClick: () => setIsCostModalOpen(true) }
        ].map((item, idx) => (
          <button 
            key={idx} 
            onClick={item.onClick}
            className="bg-[#FDFCF8] border border-[#E2D6C0] p-4 sm:p-6 rounded-[24px] flex flex-col items-center gap-3 hover:border-[#5A5A40] hover:bg-white transition-all group"
          >
      ...
      {/* Techniques Drawer */}
      <TechniquesDrawer
        isOpen={isTechniquesDrawerOpen}
        onClose={() => setIsTechniquesDrawerOpen(false)}
      />
    </div>
  );
}
